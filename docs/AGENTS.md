...
- 스타일링은 **CSS Modules** 또는 **Vanilla CSS** 사용 (TailwindCSS 사용 시 사용자 확인 필요).
- 서버 컴포넌트 우선 활용, 클라이언트 로직에만 `'use client'` 사용.

### 코드 품질
- **Encoding**: 모든 소스 코드 및 입출력 한국어는 **UTF-8** 인코딩.
- **Lint**: 주기적으로 Lint 오류 점검 및 수정.
- **Unused Code**: 사용하지 않는 파일, 의존성, 변수 제거로 프로젝트 경량화 유지.
...
2. **문서 업데이트**: `README.md` 또는 `AGENTS.md`에 변경 기능 즉시 반영 후 커밋
...
실제 코드와 이 지침(`AGENTS.md`)이 충돌하면 **지침을 최신화하는 것을 최우선**으로 제안한다.
