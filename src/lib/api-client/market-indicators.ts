 ...
 * Fallbacks to server API on web
 */
export async function fetchMarketIndicatorsClient(): Promise<MarketIndicators> {
    try {
        // Web: Use server-side API to bypass CORS
        if (!isNativeEnvironment()) {
            console.log('📊 [Client] Fetching market indicators from server API...');
            const apiUrl = typeof window !== 'undefined'
                ? `${window.location.origin}/api/market-indicators`
        ...
        let putCallRating: IndicatorRating = 'Neutral';
        let currentVIX = 20;
        let vix50DayAvg = 20;
        let vixRating: IndicatorRating = 'Neutral';
        ...
        if (response.ok) {
            ...
            if (data.fear_and_greed) {
                ...
                fearGreedRating = (data.fear_and_greed.rating as IndicatorRating) || 'Neutral';
            ...
            if (data.put_call_options?.data?.length > 0) {
                ...
                putCallRating = (data.put_call_options.rating as IndicatorRating) || 'Neutral';
            }
            ...
            if (data.market_volatility?.data?.length > 0) {
                ...
                vixRating = (data.market_volatility.rating as IndicatorRating) || 'Neutral';
